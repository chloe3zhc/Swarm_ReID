...
class build_resnet(nn.Module):
    def __init__(self, num_classes, cfg):
        ...
        self.classifier = nn.Linear(self.in_planes, self.num_classes, bias=False) # 创建全连接线性层
        self.classifier.apply(weights_init_classifier) # 初始化权重
    ...
    def forward(self, x, cam_label=None, view_label=None, label=None):  # label is unused if self.cos_layer == 'no'
        # 特征提取
        mid_fea = self.base(x) # 通过 ResNet 提取中间特征 输出维度：[batch_size, 2048, H, W]
        # 全局池化
        global_feat = self.gap(mid_fea)  # GeM全局池化，获取全局特征，维度为 [batch_size, 2048, 1, 1]
        # 展平
        global_feat = global_feat.view(global_feat.shape[0], -1)  # flatten to (batch_size, feature_dim), feature_dim=2048)
...
class FusionReID(nn.Module):
    ...
    ''''--------------------------  解决更换数据集时分类头尺寸不匹配问题：跳过分类头权重    -------------------------'''
    def load_param_ignore_classifier(self, trained_path):
        param_dict = torch.load(trained_path)
        for i in param_dict:
            if 'classifier' in i:
                print('Skipping classifier parameters:', i)
                continue
            try:
                self.state_dict()[i.replace('module.', '')].copy_(param_dict[i])
            except:
                print('Could not load parameter:', i)
        print('Loading pretrained model from {}'.format(trained_path))
