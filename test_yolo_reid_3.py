...
# 存储已知特征向量和ID
# 替换原来的 known_features 和 known_ids
memory_manager = ClusteredMemoryManager(
    max_clusters=150,
    eps=0.20,
    update_rate=0.2,
...
while True:
    ...
    if results[0].boxes is not None:
        ...
        for i, box in enumerate(results[0].boxes):
            ...
            # 提取ReID特征
            feature = extract_reid_features_batch([cropped_img], reid_model, cfg, device)[0]
            
            # 使用聚类内存管理分配ID
            reid_id = memory_manager.assign_feature(feature, current_time)
    ...
    if frame_count % 50 == 0:
        ...
        print(f"Frame {frame_count}: Clusters={stats['current_clusters']}, "
              f"Memory={stats['memory_usage_kb']:.2f}KB, "
              f"CleanupCount={stats['memory_cleanup_count']}")
