...
class MSMT17(BaseImageDataset):
    ...
    """
    # dataset_dir = 'msmt17'
    dataset_dir = 'E:\ZHC\FusionReID\MSMT17_V1'
...
class MSMT17(BaseImageDataset):
    """
    MSMT17 - Modified version for test-only data structure
    ...
    def __init__(self, root='', verbose=True, pid_begin=0, **kwargs):
        ...
        # 只加载测试数据(query 和 gallery)
        query = self._process_dir(self.test_dir, self.list_query_path)
        gallery = self._process_dir(self.test_dir, self.list_gallery_path)

        if verbose:
            print("=> MSMT17 loaded (test only)")
            self.print_dataset_statistics(query, query, gallery)  # 第一个query参数会被忽略，这里只是为了适配接口

        self.query = query
        self.gallery = gallery

        # 只计算query和gallery的统计信息
