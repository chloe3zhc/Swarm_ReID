...
def make_dataloader(cfg):
    ...
    FIXED_CAMERA_NUM = 15
    return val_loader, len(dataset.query), num_classes, FIXED_CAMERA_NUM, view_num
...
def make_dataloader(cfg):
    ...
    train_transforms = T.Compose([
        ...
        T.RandomHorizontalFlip(p=cfg.INPUT.PROB),  # 随机水平翻转（概率为cfg.INPUT.PROB）
        T.Pad(cfg.INPUT.PADDING),  # 填充
        T.RandomCrop(cfg.INPUT.SIZE_TRAIN),  # 随机裁剪到指定大小
        T.ToTensor(),  # 转换为Tensor
        T.Normalize(mean=cfg.INPUT.PIXEL_MEAN, std=cfg.INPUT.PIXEL_STD),  # 标准化
        RandomErasing(probability=cfg.INPUT.RE_PROB, mode='pixel', max_count=1, device='cpu'),  # 随机擦除数据增强
    ])
    ...
    # 使用工厂模式根据数据集名称创建数据集对象
    # __factory是一个字典，映射数据集名称到对应的数据集类
    dataset = __factory[cfg.DATASETS.NAMES](root=cfg.DATASETS.ROOT_DIR)

    # 创建训练集，应用训练数据增强
    train_set = ImageDataset(dataset.train, train_transforms)

    # 创建正常训练集（无数据增强），用于某些特定目的（如特征提取）
    train_set_normal = ImageDataset(dataset.train, val_transforms)

    # 获取训练集中的关键信息
    num_classes = dataset.num_train_pids  # 训练集中行人ID的数量（用于分类）
    cam_num = dataset.num_train_cams  # 训练集中摄像头数量（多摄像头数据）
    view_num = dataset.num_train_vids  # 训练集中视图数量（多视图数据）

    # ========================== 3. 创建训练数据加载器（根据采样器类型） ==========================

    # 判断是否使用triplet采样器（用于triplet loss）
